...
import utilities.ExtendReader;
import utilities.GlobalUtil;
import utilities.HTMLReportUtil;
import utilities.KeywordUtil;
import utilities.LogUtil;
//12import io.appium.java_client.touch.offset.PointOption;
...
public static void Back()
	...
	//mobile
	public static void clickUsingCoordinates(int xcordinate, int ycordinate) throws Exception 
	{
		TouchAction actions=new TouchAction(GlobalUtil.getMDriver());
		actions.tap(PointOption.point(xcordinate, ycordinate)).release().perform();
		
	
	}
	public static void refresh(String logStep)
	{
	...
	public static void visible(By locator, String logStep)
	{
		WebDriverWait wait = new WebDriverWait(GlobalUtil.getMDriver(), 20);
	...
	//mobile
	public static void click1(By locator, String logStep)
	{
		WebDriverWait wait = new WebDriverWait(GlobalUtil.getMDriver(), 20);
		WebElement element=wait.until(ExpectedConditions.elementToBeClickable(locator));
		element.click();
		...
		//LogUtil.infoLog(KeywordUtil.class, KeywordUtil.lastAction);
		//RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor(logStep));
		//ExtendReader.logInfo("Successfully clicked on - " + logStep);
	}
...
//mobile
	public static void swipeUp_FindElementClick(int howManySwipes, By locator, String logStep) throws InterruptedException {
		Dimension size =GlobalUtil.getMDriver().manage().window().getSize();
		...
		try {
			for (int i = 1; i <= howManySwipes; i++) {
				boolean isElmPresent =GlobalUtil.getMDriver().findElements(locator).size() > 0;
				if (isElmPresent)             
				 {
					GlobalUtil.getMDriver().findElement(locator);
					break;
				}
				TouchAction actions=new TouchAction(GlobalUtil.getMDriver());
				actions.longPress(PointOption.point(startX, startY)).moveTo(PointOption.point(startX, endY)).release().perform();
	...
	//jojo
	public static void SwipeVerticalWRElement(By locator, int swipes)
	{
		Dimension size = GlobalUtil.getMDriver().manage().window().getSize();
	...
	public static void horizontalSwipeWRTOelment(String xpath, int swipes, int startx, int endx, String logStep)
	{
		
		int y=GlobalUtil.getMDriver().findElementByXPath(xpath).getLocation().getY();
		int h=GlobalUtil.getMDriver().findElementByXPath(xpath).getRect().getHeight();
		
		size = GlobalUtil.getMDriver().manage().window().getSize();
		
		//int startx2 = (int) (size.width * startx);
		//int endx2 = (int) (size.width * endx);
		int starty2 = y+h/2;
		try {
			for (int i = 1; i <= swipes; i++) 
			{
		...
		//GlobalUtil.getMDriver().swipe(startx2, starty2, endx2, starty2, 1000);
	}
	...
	//mobile
	public static void HorizontalSwipe(int howManySwipes)
	{
		Dimension size = GlobalUtil.getMDriver().manage().window().getSize();	
...
//jojo	
	public static void swipeUp1(int howManySwipes, String logStep)
	{
		Dimension size = GlobalUtil.getMDriver().manage().window().getSize();
		// calculate coordinates for vertical swipe
		int startY = (int) (size.height * 0.7);
		...
		try {
			for (int i = 1; i <= howManySwipes; i++) 
			{
				...
				actions.longPress(PointOption.point(startX, startY)).moveTo(PointOption.point(startX, endY)).release()
						.perform();
				System.out.println("swipeUp");
				// KeywordUtil.lastAction = "swipeUp: " + howManySwipes.toString();
					LogUtil.infoLog(KeywordUtil.class, KeywordUtil.lastAction);
					RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor(logStep));
			}
		}
	...
	public static void landscapemode(String logStep)
	{
		String x=GlobalUtil.getMDriver().getOrientation().name();
		System.out.println(x);
		if(x.equalsIgnoreCase("portrait"))
		{
			GlobalUtil.getMDriver().rotate(ScreenOrientation.LANDSCAPE);
			LogUtil.infoLog(KeywordUtil.class, KeywordUtil.lastAction);
			RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor(logStep));
		}
	}
	//mobile
		public static void swipeDown(int howManySwipes,String logStep) 
		{
			Dimension size =GlobalUtil.getMDriver().manage().window().getSize();
			int startY = (int) (size.height * 0.7);
		...
		public static boolean isWebElementNotPresent(By locator) {
			...
			LogUtil.infoLog(KeywordUtil.class, KeywordUtil.lastAction);
			List<WebElement> elements = (new WebDriverWait(GlobalUtil.getMDriver(), DEFAULT_WAIT_SECONDS))
	...
	public static void scrollDown() {
		...
		for (int i = 1; i <= 9; i++) {
			...
			// GlobalUtil.mdriver.findElements(MobileBy.AccessibilityId("Add to Cart Add to
			// Shopping Cart"));
			...
			if (GlobalUtil.mdriver.findElements(MobileBy.AccessibilityId("Add to Cart Add to Shopping Cart"))
				...
				// GlobalUtil.mdriver.findElement(By.xpath("//android.widget.Button[@content-desc=\"Add
		 ...
		 * GlobalUtil.mdriver.findElement(MobileBy
